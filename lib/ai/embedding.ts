...
export const findRelevantContent = async (userQuery: string) => {
  ...
  const similarGuides = await db
    ...
    .from(embeddings)
    .where(gt(similarity, 0.3)) // Lowered threshold to allow more matches
  ...
  if (similarGuides.length === 0) {
    return { 
      results: [], 
      message: "No information found in the knowledge base. Try adding some information first using the addResource tool." 
    };
  }
